...
function InputBox({ label, amount, onAmountChange, onCurrencyChange, currencyOptions = [], selectedCurrency = "usd", amountDisabled = false, currencyDisabled = false,className=""}) {
    ...
    return (
        <div className={`bg-white p-3 rounded-lg text-sm flex ${className}`}>
            <div className='w-1-2 mr-3.5'>
                <label htmlFor={id} className='text-black/40 mb-2 absolute inline-block'>{label}</label>
                <input id= {id}
                type="number" 
                className="outline-none w-full mt-6 bg-transparent py-1.5" 
                placeholder='Amount'
